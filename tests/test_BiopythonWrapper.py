...
@pytest.mark.parametrize(
        'test_type, pdb_file, restore_folder',
        [
            ("Result after within_distance()", pdb_file, True),
            ("Result after truncate()", pdb_file, True),
        ]
)
def test_write_pdb(test_type, pdb_file, restore_file):
    ...
    assert data[0] == line0
    assert line[100] == line100

    if restore_file is True:
        restore_folder()

def test_count_atoms(pdb_file):
    model = QMzyme.GenerateModel(pdb_file)
    assert count_atoms(model) == 4258

def test_count_residues(pdb_file):
    model = QMzyme.GenerateModel(pdb_file)
    assert count_residues(model) == 324

def test_count_nonprotein_residues(pdb_file):
    model = QMzyme.GenerateModel(pdb_file)
    assert count_nonprotein_residues(model) == 62
    
def test_count_chains(pdb_file):
    model = QMzyme.GenerateModel(pdb_file)

    assert count_chains(model) == 2

def test_remove_atom(pdb_file):
    model = QMzyme.GenerateModel(pdb_file)
...
def test_get_element_idx(pdb_file):
    model = QMzyme.GenerateModel(pdb_file)
...
def test_get_atom_idx(pdb_file):
    model = QMzyme.GenerateModel(pdb_file)
    exp1 = [12, 191, 475, 1036, 1201, 1232, 1338, 1444, 1563, 1750, 2231, 2410, 2694, 3255, 3420, 3451, 3557, 3663, 3782, 3969]
    exp2 = [2003, 4222]

    assert get_atom_idx(model, 'SD') == exp1
    assert get_atom_idx(model, 'O1') == exp2

def test_make_model_dict(pdb_file):
    model = QMzyme.GenerateModel(pdb_file)
    model.set_catalytic_center(resnumber=263)
    model.within_distance(5)
    model.truncate()
    ...
    assert make_model_dict(model.models[-1]).keys() == exp_keys
