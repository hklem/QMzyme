...
from QMzyme.QMzymeBuilder_old import QMzymeModel
from QMzyme.QMzymeBuilder_old import QMzymeRegion
import inspect
import os
...
class GenerateModel(QMzymeModel):
    ...
    def within_distance(self, distance_cutoff, store_region=False, region_name=None):
        ...
        # Makes byres selection
        neighbors_byres = neighbors.residues.sorted_unique.atoms

        if region_name is None:
                region_name = f'cutoff_{distance_cutoff}'

        qmz_neighbors = QMzymeRegion(name=f'neighbors_{region_name}', atom_group=neighbors)
        qmz_neighbors_byres = QMzymeRegion(name=region_name, atom_group=neighbors_byres)
        
        qmz_neighbors_ids = [atom.id for atom in qmz_neighbors.atoms]
        for atom in qmz_neighbors_byres.atoms:
            if atom.id in qmz_neighbors_ids:
                atom.is_neighbor(True)
            else:
                atom.is_neighbor(False)

        if region_name is None:
                region_name = f'cutoff_{distance_cutoff}'

        if store_region is True:
            self.set_region(region_name, qmz_neighbors_byres)
                    
        if store_region is False:
            return (qmz_neighbors_byres)
    ...
    def truncate(self, region, scheme='CA_terminal', 
        ...
        for i, res in enumerate(residues):
            ...
            #         if atom.is_neighbor == True or res.resname == 'GLY':
            #             keep_sc = True
            # if keep_sc is True:
